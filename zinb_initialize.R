...
zinbInitialize <- function(Y, model, BPPARAM) {
    ...
    while (iter < 2) {
        ...
        alpha_mu <- matrix(unlist(bplapply(seq(n), function(i) {
            solveRidgeRegression(x = A[P[, i], , drop = FALSE], y = L[P[, i], i] - tBbeta_mu[P[, i], i], beta = alpha_mu[, i], epsilon = epsilon_alpha_mu, family = "gaussian")
        }, BPPARAM = BPPARAM)), nrow = NCOL(A))
        # beta_mu
        Aalpha_mu <- A %*% alpha_mu
        beta_mu <- matrix(unlist(bplapply(seq(m), function(i) {
            solveRidgeRegression(x = B[P[i, ], , drop = FALSE], y = L[i, P[i, ]] - Aalpha_mu[i, P[i, ]], beta = beta_mu[, i], epsilon = epsilon_beta_mu, family = "gaussian")
    ...
    while (iter < 2) {
        ...
        alpha_pi <- matrix(unlist(bplapply(seq(n), function(i) {
            solveRidgeRegression(x = A, y = Z[, i], offset = tBbeta_pi[, i], beta = alpha_pi[, i], epsilon = epsilon_alpha_pi, family = "binomial")
        }, BPPARAM = BPPARAM)), nrow = NCOL(A))
        # beta_pi
        Aalpha_pi <- A %*% alpha_pi
        beta_pi <- matrix(unlist(bplapply(seq(m), function(i) {
            solveRidgeRegression(x = B, y = Z[i, ], offset = Aalpha_pi[i, ], beta = beta_pi[, i], epsilon = epsilon_beta_pi, family = "binomial")
        }, BPPARAM = BPPARAM)), nrow = NCOL(B))
        iter <- iter + 1
    }
    zeta <- rep(0, m)

    model <- create_model(A = A, B = B, alpha_mu = alpha_mu, alpha_pi = alpha_pi, beta_mu = beta_mu, beta_pi = beta_pi, zeta = zeta, m = m, n = n, epsilon = epsilon, epsilon_alpha_mu = epsilon_alpha_mu,
