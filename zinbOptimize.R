...
zinbOptimize <- function(model, file, maxiter = 25, stop.epsilon = 0.001) {
    ...
    epsilonAlpha <- c(
        epsilon_alpha_mu, epsilon_alpha_pi
    )
    epsilonBeta <- c(
        epsilon_beta_mu, epsilon_beta_pi
    )
    Y <- as.matrix(file)
    ...
    while (iter < maxiter) {
        ...
        penalty <- sum(epsilon_alpha_mu * (alpha_mu)^2) / 2 +
            sum(epsilon_alpha_pi * (alpha_pi)^2) / 2 +
            sum(epsilon_beta_mu * (beta_mu)^2) / 2 +
            sum(epsilon_beta_pi * (beta_pi)^2) / 2 +
        ...
        estimate_alpha <- matrix(unlist(
            bplapply(seq(n), function(i) {
                optimright_fun(
                    alpha_mu[, i], alpha_pi[, i],
                    Y[, i],
                    A = A, B = B[i, ], beta_mu, beta_pi, zeta, epsilonAlpha
                )
            }, BPPARAM = BPPARAM)
        ), nrow = NROW(alpha_mu) + NROW(alpha_pi))
        ...
        ########## beta ############
        print("beta")
        estimate_beta <- matrix(unlist(
            bplapply(seq(m), function(i) {
                optimleft_fun(
                    beta_mu[, i], beta_pi[, i],
                    Y[i, ],
                    A = A[i, ], B = B, alpha_mu, alpha_pi, zeta[i], n, epsilonBeta
                )
            }, BPPARAM = BPPARAM)
        ), nrow = NROW(beta_mu) + NROW(beta_pi))
        ind <- 1
...
optimright_fun <- function(alpha_mu, alpha_pi,
                           Y, A, B, beta_mu, beta_pi, zeta, epsilonAlpha) {
    par <- c(
        alpha_mu,
        alpha_pi
    )
    ...
    optim(
        ...
        Y = Y,
        feature.mu = A,
        offset_mu = t(B %*% beta_mu),
        feature.pi = A,
        offset_pi = t(B %*% beta_pi),
        C.theta = zeta,
...
optimleft_fun <- function(beta_mu, beta_pi, Y, A, B, alpha_mu,
                          alpha_pi, zeta, n, epsilonBeta) {
    par <- c(
        beta_mu, beta_pi
    )
    par_length <- length(par)
    ...
    optim(
        ...
        Y = t(Y),
        feature.mu = B,
        offset_mu = t(A %*% alpha_mu),
        feature.pi = B,
        offset_pi = t(A %*% alpha_pi),
...
zinb.regression.parseModel <- function(para, feature.mu, offset_mu,
                                       feature.pi, offset_pi) {
    logMu <- offset_mu
    logitPi <- offset_pi
    dim.para <- rep(0, 2)
    start.para <- rep(NA, 2)
    ...
    return(list(
        logMu = logMu, logitPi = logitPi, dim.para = dim.para,
...
zinb.loglik.dispersion.gradient <- function(zeta, Y, mu, logitPi) {
    ...
    if (has0) {
        logPnb <- suppressWarnings(dnbinom(0,
            size = theta, mu = mu[Y0],
            log = TRUE
        ))
...
zinb.loglik.regression <- function(para, Y,
                                   feature.mu = matrix(nrow = length(Y), ncol = 0),
                                   offset_mu = matrix(0, nrow = length(Y), ncol = 1),
                                   feature.pi = matrix(nrow = length(Y), ncol = 0),
                                   offset_pi = matrix(0, nrow = length(Y), ncol = 1),
                                   C.theta = matrix(0, nrow = length(Y), ncol = 1), epsilon = 0) {
...
zinb.loglik.regression.gradient <- function(para, Y,
                                            feature.mu = matrix(nrow = length(Y), ncol = 0),
                                            offset_mu = matrix(0, nrow = length(Y), ncol = 1),
                                            feature.pi = matrix(nrow = length(Y), ncol = 0),
                                            offset_pi = matrix(0, nrow = length(Y), ncol = 1),
...
optimizeDispersion <- function(m, mu, logitPi, epsilon, Y) {
    g <- suppressWarnings(optimize(
        f = zinb.loglik.dispersion, Y = Y, mu = mu,
        logitPi = logitPi, maximum = TRUE, interval = c(-50, 50)
    ))
    ...
    zeta <- optim(
        par = zeta, fn = locfun, gr = locgrad,
        control = list(fnscale = -1, trace = 0, maxit = 25), method = "L-BFGS-B",
        lower = lower_bounds,
        upper = upper_bounds
    )$par
    return(zeta)
