...
get_scHiCNorm_feature <- function(address) {
    ...
    upper_tri_names <- data.frame(row = rownames(mat_map)[upper_tri_indices[, 1]], col = colnames(mat_map)[upper_tri_indices[, 2]])

    feature <- data.frame(cut = mat_cut[upper_tri_indices], gc = mat_gc[upper_tri_indices], map = mat_map[upper_tri_indices])
